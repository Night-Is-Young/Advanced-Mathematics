...
\begin{enumerate}[leftmargin=*,label=\textbf{\arabic*.},topsep=0pt,parsep=0pt,itemsep=0pt,partopsep=0pt]
    ...
    \item \textbf{(10\songti{分})}\ 在$(-1,1)$上将函数
        \[f(x)=\arctan x+\dfrac12\ln\dfrac{1+x}{1-x}\]
        展开为幂级数.

    \item \textbf{(10\songti{分})}\ 求瑕积分
    ...
    \item \textbf{(10\songti{分})}\ 设$E\in\R$.
    ...
    \item \textbf{(10\songti{分})}\ 对于每个$x\in[0,1],n=1,2,\cdots$,定义
        \[f_1(x)=\int_0^x\sqrt{1+t^4}\di t\ \ \ \ \ f_{n+1}(x)=\int_0^xf_n(t)\di t\]
        试证明
        \[\sum_{n=1}^{\infty}f_n(x)\]
        在$[0,1]$上一致收敛.

    \item \textbf{(15\songti{分})}\ 设$b\in\R$.
        \begin{enumerate}[label=\tbf{(\arabic*)},topsep=0pt,parsep=0pt,itemsep=0pt,partopsep=0pt]
